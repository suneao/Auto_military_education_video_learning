...
## 📋 修复内容
修复了原脚本中Cookie的`ZYLTheme`参数问题，解决了以下错误：
- ❌ **"1280"不是一个有效的主题名称。参数名: Theme"** 
- ✅ **修复方案**：手动设置Cookie头，避免aiohttp自动处理Cookie时导致的格式问题
- ✅ **兼容性**：支持现有Cookie和新登录获取的Cookie
...
3. **保存课程列表**：
   - 右键 → "另存为"
   - 保存为 `考试平台_files/LibraryStudyList.html`
   - 保持文件结构和命名
